describe "HandlebarsPreCompileFilter" do
  HandlebarsPreCompileFilter ||= Rake::Pipeline::Web::Filters::HandlebarsPreCompileFilter
  ...
  it "generates output" do
    filter = setup_filter HandlebarsPreCompileFilter.new
  ...
  describe "naming output files" do
    it "translates .handlebars extensions to .js by default" do
      filter = setup_filter HandlebarsPreCompileFilter.new, "test.handlebars"
      filter.output_files.first.path.should == "test.js"
    end

    it "translates .hbs extensions to .js by default" do
      filter = setup_filter HandlebarsPreCompileFilter.new, "test.hbs"
      filter.output_files.first.path.should == "test.js"
    end

    it "accepts a block to customize output file names" do
      filter = setup_filter(HandlebarsPreCompileFilter.new { |input| "squid" })
  ...
  describe "options" do
    it "should allow an option to name the key" do
      filter = setup_filter(HandlebarsPreCompileFilter.new(:key_name_proc => proc { |input| "new_name_key" }))
